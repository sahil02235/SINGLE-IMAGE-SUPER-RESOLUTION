...
from keras.layers import BatchNormalization, Activation, LeakyReLU, Add, Dense, Reshape, Lambda, Flatten, Multiply
...
def build_discriminator():
    ...
    dis1 = Conv2D(filters=256, kernel_size=9, strides=1, padding='valid')(input_layer)
    dis1 = LeakyReLU(alpha=leakyrelu_alpha)(dis1)

    dis1 = BatchNormalization(momentum=momentum)(dis1)

    k1 = Conv2D(filters=8 * 32, kernel_size=9, strides=2, padding='valid', name='primarycap_conv2')(dis1)
    k1 = Reshape(target_shape=[-1, 8], name='primarycap_reshape')(k1)
    k1 = Lambda(squash, name='primarycap_squash')(k1)
    K1 = BatchNormalization(momentum=0.8)(k1)
    k1 = Flatten()(k1)

    uhat = Dense(160, kernel_initializer='he_normal', bias_initializer='zeros', name='uhat_digitcaps')(k1)
    ...
    output = Dense(units=1, activation='sigmoid')(s_j)
    """
    ...
    k3 = Conv2D(filters=8 * 32, kernel_size=9, strides=2, padding='valid', name='primarycap_conv2')(dis3)
    k3 = Reshape(target_shape=[-1, 8], name='primarycap_reshape')(k3)
    k3 = Lambda(squash, name='primarycap_squash')(k3)

    k3 = Flatten()(k3)

    uhat_3 = Dense(640, kernel_initializer='he_normal', bias_initializer='zeros', name='uhat_digitcaps')(k3)
    c_2 = Activation('softmax', name='softmax_digitcaps1')(uhat_3)
    c = Dense(640)(c)
    x = Multiply()([uhat, c])
    s_j = LeakyReLU()(x)
